...
st.title("🛸 Drone Path Optimizer & Litchi Converter")
st.write("మీ `Centreline.kml` ఫైల్‌ను ఇక్కడ అప్‌లోడ్ చేసి Optimized KML & Litchi CSV ఫైల్స్‌ను జనరేట్ చేయండి.")

# Inputs
uploaded_file = st.file_uploader("Input KML ఫైల్‌ని అప్‌లోడ్ చేయండి", type=["kml"])
default_alt = st.number_input("Litchi Altitude (Meters)", value=30, step=1)

if st.button("🚀 Process Path & Generate Files", type="primary"):
    if uploaded_file is None:
        st.error("❌ దయచేసి KML ఫైల్‌ను అప్‌లోడ్ చేయండి.")
    else:
        try:
            ...
            OUTPUT_LITCHI_CSV = "Litchi_Waypoints.csv"
